...
import { PAGE_TEMPLATES, getSeoDefaults } from "@/lib/registry";

const SHELL_TYPES = new Set(["header", "footer"]);

function seedPagesFromTemplates() {
  const pages = {};
  for (const slug of Object.keys(PAGE_TEMPLATES)) {
    const tmpl = PAGE_TEMPLATES[slug];
    pages[slug] = {
      seo: getSeoDefaults(slug),
      blocks: (tmpl.blocks || []).filter(b => !SHELL_TYPES.has(b.type)),
    };
  }
  return pages;
}
...
const DEFAULT_CMS = {
  ...
  modals: {
    contact: {
      ...
      subjects: ["Booking Inquiry", "Property Question", "Availability Check", "Special Request", "Other"],
    },
    owner: {
      title: "List Your Property",
      successTitle: "Inquiry Received!",
      successText: "Thank you for your interest. We'll review your property details and contact you within 48 hours to discuss how we can help maximize your property's potential.",
      step1Desc: "Tell us about your property",
      step2Desc: "Your contact information",
      step3Desc: "Services you're interested in",
    },
  },
  about: {
    subtitle: "About Us",
  ...
  portfolio: {
    properties: [
      ...
      { location: "Swieqi", beds: 3, baths: 3, sleeps: 6, image: "https://primary.jwwb.nl/public/i/m/x/temp-jszjykaojetbmrgovpoe/img_0557-high.jpg" },
    ]
  },
  testimonials: [
  ...
  pages: seedPagesFromTemplates(),
  globalSeo: {
    title: "Christiano Property Management | Luxury Malta Accommodations",
    description: "Luxury short-term rental and property management across Malta. Transparent fees, no hidden markups, full-service operations for discerning property owners.",
    ogImage: "https://images.unsplash.com/photo-1771218830084-fdd272e149a1?w=1200",
  },
};
...
export const CMSProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (storedToken) {
      ...
      // Verify token server-side before granting admin access
      axios.get(`${API}/admin/stats`, { headers: { Authorization: `Bearer ${storedToken}` } })
        .then(() => setIsAdmin(true))
        .catch(() => {
          sessionStorage.removeItem("cvpm_admin_token");
          setToken("");
          setIsAdmin(false);
        });
    }
  ...
  const updateSection = useCallback(async (section, data) => {
    ...
    try {
      await axios.put(`${API}/cms/${section}`, { data }, authHeaders());
  ...
  const getPage = useCallback((slug) => {
    const stored = cms.pages?.[slug];
    if (stored?.blocks?.length) {
      return {
        seo: { ...getSeoDefaults(slug), ...stored.seo },
        blocks: stored.blocks.filter(b => !SHELL_TYPES.has(b.type)),
      };
    }
    const tmpl = PAGE_TEMPLATES[slug];
    if (tmpl) {
    ...
    return { seo: getSeoDefaults(slug), blocks: [] };
  }, [cms.pages]);

  const savePageBlocks = useCallback(async (slug, blocks, seo) => {
    if (!token) return false;
    try {
      const pageSeo = seo ?? cms.pages?.[slug]?.seo ?? getSeoDefaults(slug);
      await axios.put(`${API}/cms/pages/${slug}`, { blocks, seo: pageSeo }, authHeaders());
      setCms(prev => ({
        ...prev,
        pages: {
          ...prev.pages,
          [slug]: { ...(prev.pages?.[slug] || {}), blocks, seo: pageSeo },
        },
      }));
      return true;
    } catch {
  ...
  }, [token, authHeaders, cms.pages]);

  const publishPage = useCallback(async (slug) => {
    if (!token) return false;
    try {
      await axios.post(`${API}/cms/pages/${slug}/publish`, {}, authHeaders());
      return true;
    } catch {
      return false;
    }
  ...
  const verifyAdmin = useCallback(async (password) => {
    try {
      const response = await axios.post(`${API}/auth/login`, { password });
      const jwtToken = response.data.token;
      setToken(jwtToken);
      sessionStorage.setItem("cvpm_admin_token", jwtToken);
      setIsAdmin(true);
      return true;
    } catch {
      setIsAdmin(false);
  ...
  return (
    <CMSContext.Provider value={{
      cms,
      globalSeo: cms.globalSeo || cms.seo || DEFAULT_CMS.globalSeo,
      isLoading,
      isAdmin,
      token,
      updateSection,

...
import { API, STRIPE_PUBLISHABLE_KEY } from "@/lib/api";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6" data-testid="stripe-elements-form">
      <div className="bg-[#161618] border border-white/10 p-6 md:p-8">
        <h2 className="heading text-xl text-[#F5F5F0] mb-4 flex items-center gap-3">
...
export const StripeInlinePayment = ({
  ...
  useEffect(() => {
    if (!PUBLISHABLE_KEY) {
      setLoadError("Stripe is not configured (VITE_STRIPE_PUBLISHABLE_KEY missing).");
      return;
    }
    let cancelled = false;
    (async () => {

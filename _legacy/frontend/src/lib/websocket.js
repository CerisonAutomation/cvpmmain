...
const wsBase = BACKEND_URL || (typeof window !== 'undefined' ? window.location.origin : '');
const WS_URL = wsBase.replace('/api', '').replace('http', 'ws');
...
class WebSocketManager {
  ...
  startPolling(fetchDraft, intervalMs = 5000) {
    this.stopPolling();
    this.usePolling = true;
    this.pollTimer = setInterval(async () => {
      try {
        const data = await fetchDraft();
        if (data) this.trigger("draft_sync", data);
      } catch {}
    }, intervalMs);
  }
  ...
  connect(user = 'Admin') {
    ...
    const origin = wsBase || (typeof window !== "undefined" ? window.location.origin : "");
  ...
  joinPage(pageId, user = 'Admin') {
    if (!this.connected) {
      // Not connected, queuing join request
      return;
    }

...
import {
  ...
  Cast, ScreenShare, PictureInPicture, Fullscreen,
  Layers, LayoutDashboard, PanelLeft, PanelRight, PanelTop, PanelBottom,
  Table, TableProperties, ToggleLeft, ToggleRight,
  ...
  Hammer, Wrench, Paintbrush, Palette, Scissors,
  ...
  CircleParking, CircleParkingOff,
  Heater
} from "lucide-react";
...
export const getAmenityIcon = (amenityKey) => {
  const normalizedKey = amenityKey?.toUpperCase().replace(/[- ]/g, '_');
  return AMENITY_ICONS[normalizedKey] || { icon: CheckCircle, label: formatAmenityLabel(amenityKey), category: "other" };
};

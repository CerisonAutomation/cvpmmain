...
export const PropertiesPage = () => {
  ...
  const textSearch = (searchParams.get("search") || "").toLowerCase();
  ...
  const sortedListings = useMemo(() => {
    ...
    if (textSearch) {
      sorted = sorted.filter((l) => {
        const title = (l.title || l.nickname || "").toLowerCase();
        const city = (l.address?.city || "").toLowerCase();
        return title.includes(textSearch) || city.includes(textSearch);
      });
    }
    switch (sortBy) {
      case "price-low":
  ...
  }, [listings, sortBy, textSearch]);

  useEffect(() => {
    trackEvent(AnalyticsEvents.VIEW_PROPERTIES);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#0F0F10] pt-24 pb-16" data-testid="properties-page">
      <SEO title="Properties" description={cms.seo?.description} keywords={cms.seo?.keywords} pageSeo={cms.pages?.properties?.seo} />
      {/* Header */}
      <div className="bg-[#0A0A0B] border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 py-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
              <h1 className="heading text-3xl md:text-4xl text-[#F5F5F0] mb-2">
      ...
      {!loading && !error && sortedListings.length > 0 && (
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 mt-12">
          <div className="bg-[#161618] border border-white/5 p-8 md:p-12 text-center">
            ...
            <h2 className="heading text-2xl md:text-3xl text-[#F5F5F0] mb-3">
              Explore Map
            </h2>

    ...
    <div className="h-14 flex items-center gap-3 px-4 bg-card border-b border-border">
      ...
      <div className="ml-auto flex items-center gap-1">
        ...
        {(["desktop", "tablet", "mobile"] as const).map((d) => {
          ...
          return (
            <button
              ...
              className={`p-1.5 rounded ${state.device === d ? "bg-primary/10 text-primary" : "hover:bg-muted text-muted-foreground"}`}
            >
        ...
        <button
          onClick={onSave}
          disabled={saving}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded bg-primary text-primary-foreground text-xs font-medium disabled:opacity-50"
        >
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {cat.blocks.map((b) => (
                <button
                  ...
                  className="p-2 bg-muted/40 hover:bg-muted border border-border rounded text-center transition"
                >
...
function Canvas() {
  ...
  return (
    ...
    <main className="flex-1 overflow-y-auto bg-background p-6">
      <div
        className="mx-auto bg-background border border-border rounded-lg shadow-2xl transition-all"
        style={{ maxWidth: w }}
      >
...
function PropsPanel() {
  ...
  return (
    ...
    <aside key={selected.id} className="w-80 bg-card border-l border-border overflow-y-auto">
      <div className="p-4 space-y-4">
        <div>
          <div className="text-[10px] font-bold tracking-[0.16em] uppercase text-muted-foreground border-b border-border pb-2 mb-3">
            {selected.type} block
          </div>
                  ...
                  <div key={k}>
                    ...
                    {isLong ? (
                      <textarea
                        ...
                        onBlur={(e) => dispatch({ type: "PATCH", id: selected.id, data: { [k]: e.target.value } })}
                        className="w-full px-2 py-1.5 bg-muted/40 border border-border rounded text-xs min-h-[60px]"
                      />
                    ) : (
                      <input
                        ...
                        onBlur={(e) =>
                          dispatch({
                            type: "PATCH",
                            id: selected.id,
                            data: { [k]: typeof v === "number" ? Number(e.target.value) : e.target.value },
                          })
                        }
                        className="w-full px-2 py-1.5 bg-muted/40 border border-border rounded text-xs"
                      />
                    )}
                  </div>
                );
        ...
        <div className="border-t border-border pt-4">
          <div className="flex items-center gap-1.5 text-[10px] font-bold tracking-[0.16em] uppercase text-muted-foreground mb-2">
          ...
          <button
            onClick={onAi}
            disabled={aiBusy}
            className="mt-2 w-full inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded bg-primary text-primary-foreground text-xs disabled:opacity-50"
          >
...
function Toast() {
  ...
  return (
    <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 px-4 py-2 rounded shadow-xl text-white text-xs ${bg} z-50`}>
      {toast.msg}
    </div>
  );
}
        ...
        <div className="bg-card border border-border rounded-lg p-6 mb-6">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 bg-muted/40 border border-border rounded text-sm"
            />
            <input
              placeholder="slug"
              value={slug}
